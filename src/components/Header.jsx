...
const Header = ({ toggleSidebar, toggleNotifications }) => {
   ...
   const getPageTitle = () => {
      ...
      return routeTitles[segment] || "Dashboard";
   };
   ...
   return (
      <header className="header h-16 flex items-center justify-between px-6 sticky top-0 z-10 border-b">
         <div className="flex items-center">
            ...
            <div className="flex items-center">
               <span className="text-gray-500 mr-2 font-sf-medium">EDUNITE</span>
               <span className="text-gray-400 mx-2">/</span>
               <span className="font-sf-regular">{getPageTitle()}</span>
            </div>
         </div>
         ...
         <div className="flex items-center space-x-3">
            ...
            <div className="flex items-center ml-2 relative" ref={userMenuRef}>
               <button className="flex items-center focus:outline-none" onClick={() => setUserMenuOpen(!userMenuOpen)}>
                  <div className="w-8 h-8 bg-primary-100 text-primary-600 rounded-full flex items-center justify-center">
                     <FiUser size={16} />
                  </div>
                  {user && <span className="ml-2 text-sm font-sf-regular hidden md:block">{user.name}</span>}
               </button>
               ...
               {userMenuOpen && (
                  <div className="absolute right-0 top-full mt-2 w-48 card rounded-md shadow-lg py-1 z-20 border">
                     <div className="px-4 py-2 border-b">
                        <p className="text-sm font-sf-regular">{user?.name}</p>
                        <p className="text-xs text-tertiary truncate font-sf-regular">{user?.email}</p>
                     </div>

                     <button
                        onClick={handleLogout}
                        className="flex w-full items-center px-4 py-2 text-sm hover:bg-tertiary font-sf-regular"
                     >
