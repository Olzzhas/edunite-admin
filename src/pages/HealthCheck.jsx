...
import { FiRefreshCw, FiCheckCircle, FiXCircle, FiAlertTriangle, FiClock, FiServer, FiDatabase, FiUsers, FiLock, FiHardDrive, FiActivity, FiAward } from "react-icons/fi";
...
const HealthCheck = () => {
  ...
  const checkServicesHealth = () => {
    ...
    // Update services with random statuses for mock data
    const updatedServices = services.map(service => {
      // Generate random status: 80% chance of healthy, 15% degraded, 5% down
    ...
    if (updatedServices.some(service => service.status === "down")) {
      setOverallStatus("degraded");
    } else if (updatedServices.every(service => service.status === "healthy")) {
          ...
          <div className="flex items-center">
            ...
            <span className="font-medium">
              {overallStatus === "healthy" ? "All Systems Operational" : 
               ...
               overallStatus === "down" ? "Major System Outage" : "Checking Status..."}
            </span>
          </div>
        </div>
      </div>
